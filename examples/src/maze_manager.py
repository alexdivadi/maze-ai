...
from src.solver import DepthFirstBacktracker, BiDirectional, BreadthFirst, UCS, AStar, QLearning
...
class MazeManager(object):
    ...
    def solve_maze(self, maze_id, method, neighbor_method="fancy", a=1, g=1, e=0, max_iter=1000):
        ...
        if method == "DepthFirstBacktracker":
            ...
            solver = UCS(maze, neighbor_method, self.quiet_mode)
            maze.solution_path = solver.solve()
            solver.get_optimal_path()
        elif method == "a_star_search":
            solver = AStar(maze, neighbor_method, self.quiet_mode)
            maze.solution_path = solver.solve()
            solver.get_optimal_path()
        elif method == "q_learning_search":
            solver = QLearning(maze, neighbor_method, self.quiet_mode, a, g, e, max_iter)
            maze.solution_path = solver.solve()
    ...
    def reset(self, maze_id):
        """
        Resets all values in maze so it can be solved again
        """
        maze = self.get_maze(maze_id)
        for x in range(maze.num_rows):
            for y in range(maze.num_cols):
                maze.grid[x][y].visited = False
                maze.grid[x][y].parent = None
                maze.grid[x][y].cost = float('inf')
        maze.solution_path = None
        maze.optimal_path = None
        maze.optimal_path_cost = 0
        maze.q_table = maze.generate_q_table()
