...
class Maze(object):
    ...
    def validate_neighbours_solve(self, neighbour_indices, k, l, k_end, l_end, method = "fancy"):
        ...
        # Set parent of each neighbor node
        for n in neigh_list:
            self.grid[n[0]][n[1]].parent = self.grid[k][l]
    ...
    def euclidean_distance(k_n, l_n, k_end, l_end):
        """Returns the Euclidean distance between two cells"""
        return math.sqrt((k_n - k_end) ** 2 + (l_n - l_end) ** 2)

    def is_wall(self, current_state, next_state):
        """Returns True if there is a wall between two states or next_state 
        is out of bounds"""
        if next_state[0] >= self.num_rows or next_state[0] < 0 \
            or next_state[1] >= self.num_cols or next_state[1] < 0:
            return True
        return self.grid[current_state[0]][current_state[1]].is_walls_between(
            self.grid[next_state[0]][next_state[1]]) 
